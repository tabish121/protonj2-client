...
public class ConnectionOptions {
    ...
    private BiConsumer<Connection, ConnectionEvent> interruptedHandler;
    private BiConsumer<Connection, ConnectionEvent> reconnectedHandler = (connection, remoteUri) -> {
        if (connectedhedHandler != null) {
            connectedhedHandler.accept(connection, remoteUri);
        }
    };
     ...
     */
    public BiConsumer<Connection, ConnectionEvent> interruptedHandler() {
     ...
     * @see #failedHandler
     */
    public ConnectionOptions interruptedHandler(BiConsumer<Connection, ConnectionEvent> interruptedHandler) {
     ...
     * configuration.  If a handler is not configured here then each connection to a remote peer
     * is forwarded to the configured {@link #connectedHandler()} if one has been configured.
     *
